...
There is no per-person filtering; if you need one, that is a feature to build, not a
setting to find.
"""
...
def cmd_list(store: subscribers.Store) -> int:
    ...
    for s in subs:
        ...
        print(f"  [{state}] {s.name:<20} {s.chat_id:<16} added {s.added_at or '?'}{why}")
    return 0
...
def cmd_pending(store: subscribers.Store) -> int:
    ...
    print(f"\nAdd one with:\n  python3 tools/telegram_subscribers.py --add {example} --name <who>")
    return 0


def cmd_add(store: subscribers.Store, chat_id: str, name: str | None) -> int:
    owner = _owner()
    ...
    try:
        sub = store.add(chat_id, name or "")
    except subscribers.SubscriberError as e:
        print(e, file=sys.stderr)
        return 2
    print(f"added: {sub.name} ({sub.chat_id}) — receives every alert for every target")
    print(f"Prove it reaches them:\n"
...
def cmd_test(chat_id: str) -> int:
    ...
    alert = Alert(target_id="test", rule="test_notify", headline="TEST — R$ 123,45",
                  detail="Synthetic alert from price-watcher. If you received this, "
                         "you are on the list and delivery works.",
                  reading=probe)
    ...
    with tempfile.TemporaryDirectory() as d:
        notifier = TelegramNotifier(chat_id=str(chat_id),
        ...
        try:
            ...
            return 3
    print(f"sent to {chat_id}. Confirm it ARRIVED — a clean send is not a delivered message.")
    return 0
...
def main(argv=None) -> int:
    ...
    try:
        ...
        if args.add:
            return cmd_add(store, args.add, args.name)
        if args.remove:
